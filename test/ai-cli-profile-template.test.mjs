...
test('profile center presents one read-only service profile section with model-aware defaults', () => {
  const page = read('../src/views/ProfileCenter.vue')
  const card = read('../src/components/profiles/ServerServiceProfileCard.vue')

  assert.match(page, /服务档案/)
  assert.match(page, /service-profile-card-grid/)
  assert.match(page, /v-for="profile in profiles\.serviceProfiles"/)
  assert.doesNotMatch(page, /visibleProfiles\.length[\s\S]*source === 'server'/)
  assert.match(page, /选择服务档案默认模型/)
  assert.match(page, /v-model:value="selectedServiceProfileId"/)
  assert.match(page, /v-model:value="selectedServiceModelId"/)
  assert.match(page, /:disabled="!serviceDefaultSelection\.valid"/)
  assert.match(page, /model:\s*selectedServiceModelId\.value/)
  assert.doesNotMatch(page, /compatibleServiceModels\.value\[0\]|models\[0\]/)
  assert.match(page, /modelCatalogError/)
  assert.match(page, /retryModelCatalog/)
  assert.match(card, /profile\.models/)
  assert.match(card, /describeModelProtocols/)
  assert.match(card, /contextSize/)
  assert.match(card, /availabilityStatus/)
  assert.doesNotMatch(card, /revision|digest|credential|secret|artifactId/)
})
