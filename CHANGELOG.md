...
- 配置档案页的服务档案区域新增独立“同步服务档案”入口，可直接刷新服务端模型能力目录，无需再跳转到“设置 → 服务端连接”。
- 服务档案同步只刷新连接状态和模型目录，不隐式同步组织 Skills；Skills 页面继续提供独立的“同步组织目录”入口。
- 同步后立即按服务端声明的协议能力刷新模型可用性，`openai_responses` 等协议不依赖模型顺序、模型 ID 或厂商猜测。

### 依赖安全

- 将构建链的 `@xmldom/xmldom`、`fast-uri` 与运行时传递依赖 `qs` 固定到已修复的兼容补丁版本，发布依赖审计恢复为零已知漏洞。
- 保留 npm 11.16 所需的 Windows 签名工具 optional peer 锁文件记录，使本地 npm 11.6 与 GitHub runner 的干净安装结果一致。

## [0.12.2] - 2026-09-03（未发布）

- `v0.12.2` 在 GitHub Actions 的 `npm ci` 前置门失败，没有执行测试、构建或发布，也没有创建 GitHub Release；修复锁文件跨 npm 版本兼容后由 `0.12.3` 取代。

## [0.12.1] - 2026-09-01

### Skills 管理

- Skills 页面分为“全部”、“组织 Skills”和“本地 Skills”：组织目录先显示安全缓存，再在后台按五分钟 TTL 刷新；匹配的目录变更事件会刷新当前页面。临时网络失败保留最后成功目录，显式断开只清除未安装的在线目录。
- 已安装组织 Skill 持久保存规范化服务端、组织和目录版本来源身份；即使断开或目录暂不可用，仍归属原组织而不会落入本地来源。
- CLI 可见性改为 `enabled`、`disabled`、`inherit` 的期望状态。无法可靠排除继承目录时，严格停用会以 `SKILL_CLI_ISOLATION_UNSUPPORTED` 阻止；提供者停用会先为仍启用的消费者迁移并校验投影。
- 已安装 Skills 支持当前筛选范围内的批量安装、更新、启用、停用和移除。每项独立返回成功、失败、跳过或恢复结果；持久化待确认或恢复失败会停止剩余项。
- 停用仅改变 CLI 投影状态；移除投影不删除规范包；“移除受管包”是单独确认的危险操作，才会删除规范副本及其管理元数据。

## [0.12.0] - 2026-08-28

### 服务端接入

- 新增 Device Grant Link v1 的服务端注册、凭证刷新与生命周期状态；未连接、不可达或授权失效时，本地模型、已安装 Skills、会话和数据保持独立可用。
- 服务端模型经本机回环代理提供为只读组织档案，不自动设为默认模型；服务端 Skills 只提供目录和用户显式安装入口，不自动安装或更新。
- 设置页支持受控的 `ucli://` 连接入口；Windows 安装版声明协议支持，portable 保持不接管已安装版协议归属。
- 固定合约 fixtures 覆盖设备注册、刷新、Bootstrap、Skills、稳定错误与模型 SSE 转发；真实内网冒烟仍需一次性新授权后显式启用。

### 发布与回滚边界

- 紧急关闭只移除服务端入口和能力，不删除本地会话、档案、Skills 或数据；真实 0.11.6 二进制降级仍是发布前人工门禁。

## [0.11.6] - 2026-08-25

### 工作总结闭环

- 移除工作总结的双真相源：版本化数据库报告及其规范 Markdown 成为唯一业务事实，预览和导出均按选定报告派生。
- 修复同周期重跑覆盖历史、既有 HTML 目标误判成功与报告/会话历史错绑问题；每个 run 使用独立报告版本、工作区和会话。
- 提示词投递以 `turn_started` 为确认门槛，状态卡死、关闭和恢复路径均可收口到安全终态，旧 `workLogs` 只做只读幂等导入。
...
- 增加报告导出、状态恢复、迁移、挂载视图和发布门禁测试；DSH bridge 打包测试使用隔离输出目录，Windows 符号链接能力不可用时只跳过创建能力测试。

## [0.11.5] - 2026-08-20

### 修复

- 修复「弹出窗口」打开的是完整主应用而非产物内容页的问题：等待路由首次导航完成后再判断预览路由，避免被持久化工作台状态重定向顶掉。
- 修复 HTML 产物预览失败的问题：改为在 `sandbox="allow-same-origin"` 的 iframe `srcdoc` 中隔离渲染，作为真正页面呈现。
- 产物列表增加类型标签（图片 / Markdown / HTML / 文本）与类型筛选；抽屉与弹出窗口复用同一「筛选 + 目录 + 预览」组件，弹出窗口同时获得左侧目录与筛选。
