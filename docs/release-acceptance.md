...
| Windows 安装器修复候选 | PASS：2026-08-30 21:37（Asia/Shanghai）在 Windows x64 上从最终修复源码构建。Setup `UCLI-Setup-0.12.0-x64.exe`（134,216,865 bytes，SHA-256 `B2383144D34212DFC1527E93F9A67A7FBF418029B46E3ADB5D06524A74B052EA`）；Portable `UCLI-Portable-0.12.0-x64.exe`（133,961,402 bytes，SHA-256 `22B2D62F90D1B10ECA06DAE62365A68DFC62A4B74F59D4997E2E3186594D65EA`）。`npm run verify:release` 通过；核心修复后全量测试 1881 项、1869 通过、12 跳过、0 失败，最终安装页 PRE 组合回调目标测试 7/7 通过。 |
| Windows 原生人工检查 | 部分 PASS：在已运行的安装版 `0.12.0` 上覆盖安装上述 Setup；点击进入安装阶段后 5 秒内 `UCLI.exe` 进程归零，未出现“无法关闭”，安装完成并自动启动。界面仍显示 15 个项目、34 个会话和 `v0.12.0`；卸载注册为 `UCLI 0.12.0`，`ucli://` 命令为 `"F:\soft\ucli\UCLI.exe" "%1"`，作用域标记存在。根因为 32 位 NSIS 启动的 PowerShell 无法可靠读取 64 位 Electron 的 `Get-Process.Path`；修复后以 CIM `ExecutablePath` 枚举，并新增 32/64 位、路径隔离、legacy 同名进程和 replacement race 回归。仍待完成：cold start、第二实例、条件卸载，以及 portable 不接管协议。 |
